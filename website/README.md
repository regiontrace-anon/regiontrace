...
Companion website for an anonymous submission to ICLR 2027. Research code will be released separately in [`../code/`](../code/).
...
Requires Node.js 22.13 or newer. Run these commands from this `website/` directory.
...
The site remains publicly accessible at https://regiontrace-anon.github.io/regiontrace/. The GitHub Actions workflow in [`../.github/workflows/deploy.yml`](../.github/workflows/deploy.yml) builds from `website/` and deploys `website/dist/` on every push to `main`. In repository Settings → Pages, the source is GitHub Actions. The Vite base path is `/regiontrace/`; update it if you deploy under a different repository name.
