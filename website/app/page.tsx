...
import { ArrowUpRight, ArrowRight, Scan, MoveUpRight, Download, Code2, RotateCcw, ChevronRight } from 'lucide-react';
...
import References, {Cite} from './references';
...
export default function Home(){
 ...
 useEffect(()=>{
  ...
  const sections=[['overview',''],['explorer','method'],['method','method'],['studies','studies'],['findings','studies'],['toolbox','toolbox'],['references','toolbox']];
  let frame=0;
  const update=()=>{
   frame=0;
   ...
   let current='';
   for(const [id,nav] of sections){const el=document.getElementById(id);if(el&&el.getBoundingClientRect().top<=height+36)current=nav;}
   setActiveNav(current);
  };
  const queue=()=>{if(!frame)frame=requestAnimationFrame(update)};
  const observer=new ResizeObserver(queue);observer.observe(document.body);if(header)observer.observe(header);
  window.addEventListener('scroll',queue,{passive:true});window.addEventListener('resize',queue);window.addEventListener('hashchange',queue);update();
  return ()=>{cancelAnimationFrame(frame);observer.disconnect();window.removeEventListener('scroll',queue);window.removeEventListener('resize',queue);window.removeEventListener('hashchange',queue);document.documentElement.style.removeProperty('--nav-height')};
 },[]);
 ...
 return <>
 <SectionNavigation/>
 <header id="site-navigation" className="nav"><a className="brand" href="#"><Scan size={27}/><span>RegionTrace<span className="brand-dot">.</span></span></a><nav aria-label="Main navigation"><a href="#method" aria-current={activeNav==='method'?'location':undefined}>How it works</a><a href="#studies" aria-current={activeNav==='studies'?'location':undefined}>Studies</a><a href="#toolbox" aria-current={activeNav==='toolbox'?'location':undefined}>Toolbox</a></nav><div role="group" aria-label="Project links" style={{display:"flex",flexWrap:"wrap",justifyContent:"flex-end",gap:12}}><a className="paper-link" href="https://github.com/regiontrace-anon/regiontrace" target="_blank" rel="noreferrer"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 .297C5.37.297 0 5.67 0 12.297c0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.043-1.61-4.043-1.61-.546-1.387-1.333-1.756-1.333-1.756-1.09-.745.083-.73.083-.73 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.807 1.305 3.492.998.108-.776.418-1.305.762-1.605-2.665-.3-5.467-1.334-5.467-5.93 0-1.31.467-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23a11.5 11.5 0 0 1 3-.404c1.02.005 2.045.138 3 .404 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg> GitHub <ArrowUpRight size={15}/></a><a className="paper-link" href="https://github.com/regiontrace-anon/regiontrace/tree/main/code" target="_blank" rel="noreferrer"><Code2 size={17}/> Code <ArrowUpRight size={15}/></a></div></header>
 <main>
 ...
 <div className="explorer intro-explorer"><div className="case-grid"><div className="case-visual"><div className="panel-label">COCO 2017 / IMAGE 81061<Cite id="coco"/> <span>FIG. 1</span></div><img className="spatial-image" src={`assets/${key}.png`} alt={`Region combination ${key}: ${score.visible_regions.join(', ')||'all selected regions hidden'}`}/><div className="region-switches">{['Suitcase','Couch','Chair'].map((r,i)=><label key={r}>{r}<Switch aria-label={`Show ${r}`} checked={visible[i]} onCheckedChange={v=>setVisible(prev=>prev.map((p,j)=>j===i?v:p))}/></label>)}</div></div><div className="case-analysis"><span className="tag">COMPARE VISUAL CONTEXTS</span><h3>A region’s effect depends<br/>on what else is visible.</h3><p className="muted">Is the suitcase in front of the couch?<br/>Fixed target: Yes · InternVL3-8B</p><div className="score-display" aria-live="polite"><span>Support for Yes in this combination</span><strong>{(score.conditional_probability_yes*100).toFixed(1)}<small>%</small></strong><p>Yes–No log-score margin <b>{score.yes_minus_no_margin>0?'+':''}{score.yes_minus_no_margin.toFixed(3)}</b> · Constrained answer <b>{score.binary_argmax==='yes'?'Yes':'No'}</b></p></div><div className="insight"><Scan size={20}/><p>Start with the suitcase and couch. Hide one, then restore it and watch the score change. The method below explains how to measure this difference and account for interactions.</p></div><button className="download-link" onClick={()=>setVisible([true,true,true])}><RotateCcw size={15}/> Restore all regions</button><a className="download-link" href="assets/spatial-case.json" download><Download size={15}/> Download all eight combinations</a><p className="fineprint">Hidden regions use the processor-mean baseline. Pixels outside the selected regions remain unchanged.</p></div></div></div>
 <a className="text-link intro-next" href="#method">How do we turn these comparisons into an explanation? <ArrowRight size={17}/></a>
 </section>
 <MethodWalkthrough/>
 <section id="studies" className="wrap section"><div className="section-heading"><div><div className="eyebrow">THREE STUDIES · THREE USES OF REGIONTRACE</div><h2>From the method to the research.</h2></div><div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:16}}><p>Explore a case from each study in the paper.<br/>Every interaction replays precomputed data.</p></div></div>
 <p className="study-sources">Study I uses PTR<Cite id="ptr"/> and PACO<Cite id="paco"/>. Studies II and III use MM-GCoT<Cite id="mmgcot"/> and VCR<Cite id="vcr"/>. The evaluated model families include InternVL3<Cite id="internvl"/>, Qwen2.5-VL<Cite id="qwen"/>, and Look-Back<Cite id="lookback"/>.</p>
